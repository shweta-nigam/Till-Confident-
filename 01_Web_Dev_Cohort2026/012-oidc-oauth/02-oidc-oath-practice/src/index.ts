...
app.get("/o/authorize", async (req, res) => {
  ...
  if (!user) {
    return res.redirect(
      `/o/authenticate?client_id=${encodeURIComponent(client_id)}&redirect_uri=${encodeURIComponent(redirect_uri)}&state=${encodeURIComponent(state)}`
    );
  }

  //  pass state to consent page
  return res.redirect(
    `/consent.html?client_id=${encodeURIComponent(client_id)}&redirect_uri=${encodeURIComponent(redirect_uri)}&app_name=${encodeURIComponent(appData.name)}&state=${encodeURIComponent(state)}`
  );
})
